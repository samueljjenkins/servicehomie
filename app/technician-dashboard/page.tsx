...
export default function TechnicianDashboard() {
  ...
  const profile = {
    ...
    business_name: "Carter's Window Cleaning",
  };
  const earnings = 2450.75;
  const jobsCompleted = 38;

  // Mock earnings breakdown
  const earningsThisWeek = 620.0;
  const earningsThisMonth = 1640.0;
  const mostBookedService = "Window Cleaning";
  const nextPayout = "Friday, July 25";
    ...
    <section className="py-20 min-h-[80vh] bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 text-center">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-blue-700 mb-2 drop-shadow">Welcome, {profile.full_name}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 text-center flex flex-col items-center">
            <div className="text-4xl font-extrabold text-blue-600 mb-2">{jobsCompleted}</div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-left">
            <div>
              ...
              <p className="text-xl font-semibold text-green-600">${earningsThisWeek.toFixed(2)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 mb-1">Earnings This Month</p>
              <p className="text-xl font-semibold text-green-600">${earningsThisMonth.toFixed(2)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 mb-1">Most Booked Service</p>
              <p className="text-xl font-semibold text-blue-600">{mostBookedService}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 mb-1">Next Payout</p>
              <p className="text-xl font-semibold text-purple-600">{nextPayout}</p>
            </div>
          </div>
        </div>
      </div>
